...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf &kf_state, int &N)  // n最大迭代次数，即开头的多少帧imu拿来初始化
{
  //MeasureGroup这个struct表示当前过程中正在处理的所有数据，包含IMU队列和一帧lidar的点云 以及lidar的起始和结束时间
  //初始化重力、陀螺仪偏差、acc和陀螺仪协方差  将加速度测量值归一化为单位重力   **/
  V3D cur_acc, cur_gyr;            //当前的加速度、当前的角速度
  ...
  for (const auto &imu : meas.imu)    //根据所有IMU数据，计算平均值和方差//遍历一个名为meas.imu对象
  {
    ...
    //由数据和均值计算方差的公式
    cov_acc = cov_acc * (N - 1.0) / N + (cur_acc - mean_acc).cwiseProduct(cur_acc - mean_acc)  / N;   //对两个矩阵元素进行逐行元素相乘
  ...
  init_state.grav = - mean_acc / mean_acc.norm() * G_m_s2;    //得平均测量的单位方向向量 * 重力加速度预设值   norm为2范数
...
void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf &kf_state, PointCloudXYZI &pcl_out)
{
  ...
  pcl_out = *(meas.lidar);                                        //拿出来雷达帧
  sort(pcl_out.points.begin(), pcl_out.points.end(), time_list);  //sort 函数需要接收一个比较函数作为第三个参数，该比较函数用于确定元素之间的顺序，这里curvature中存放了时间戳（在preprocess.cpp中）


  state_ikfom imu_state = kf_state.get_x();                       // 获取上一次KF估计的后验状态作为本次IMU预测的初始状态
  IMUpose.clear();
  IMUpose.push_back(set_pose6d(0.0, acc_s_last, angvel_last, imu_state.vel, imu_state.pos, imu_state.rot.matrix()));       //6d 表示6个自由度
  ...
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++)
  {
    ...
    in.gyro = angvel_avr;
    Q.block<3, 3>(0, 0).diagonal() = cov_gyr;         // 配置协方差矩阵，这些协方差在初始化的时候就设置好了
...
//通过函数进行一个imu的前向传播，把前向传播的状态量赋给kf_state，把运动补偿后的点云赋给cur_pcl_un_
double T1,T2;          //三个输入，第一个是打包好的测量值 雷达及雷达帧间的imu数据 第二个是esekf对象，里面存储了待优化量x_和p_ 第三个输入是当前未去畸变的点云
void ImuProcess::Process(const MeasureGroup &meas, esekfom::esekf &kf_state, PointCloudXYZI::Ptr &cur_pcl_un_)//第三个是我们的输出量
{
  // T1 = omp_get_wtime();

  if(meas.imu.empty()) {return;};                   //判断meas是否为空
  ROS_ASSERT(meas.lidar != nullptr);

  if (imu_need_init_)                              //如果imu需要初始化，第一帧默认需要初始化
  {
    // The very first lidar frame
    IMU_init(meas, kf_state, init_iter_num);       //如果开头几帧  需要初始化IMU参数 num=1
