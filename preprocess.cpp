...
void Preprocess::process(const sensor_msgs::PointCloud2::ConstPtr &msg, PointCloudXYZI::Ptr &pcl_out)
{
  ...
  *pcl_out = pl_surf;                            //将类型转换好的面点赋值给pcl_out
}
...
{
  pl_surf.clear();                                                                        //清空一下这里面的点云

  pcl::PointCloud<rslidar_ros::Point> pl_orig;                                            //定义一个传感器类型的点云
  pcl::fromROSMsg(*msg, pl_orig);                                                         //把ros消息msg转变成该传感器类型的点云pl_orig
  int plsize = pl_orig.points.size();                                                     //获取点云的大小
  pl_surf.reserve(plsize);                                                                // 这行代码的作用是为名为 pl_surf 的 std::vector 对象预留 plsize 个元素的存储空间，但不会实际插入元素
  ...
  for (int i = 0; i < plsize; i++)
  {
    PointType added_pt;                           //pcl标准点类型          

    added_pt.normal_x = 0;
    added_pt.normal_y = 0;
    added_pt.normal_z = 0;
    added_pt.x = pl_orig.points[i].x;             //将雷达点云赋值给标准pcl雷达点
    added_pt.y = pl_orig.points[i].y;
    added_pt.z = pl_orig.points[i].z;
    added_pt.intensity = pl_orig.points[i].intensity;
    added_pt.curvature = (pl_orig.points[i].timestamp - pl_orig.points[0].timestamp) * 1000.0; // 相对时间 ms
    // std::cout << "added_pt.curvature:" << added_pt.curvature << std::endl;

    if (!given_offset_time)                                                //如果时间戳为FALSE的情况下
    {
      int layer = pl_orig.points[i].ring;
      double yaw_angle = atan2(added_pt.y, added_pt.x) * 57.2957;
    ...
    if (i % point_filter_num == 0)                                                                          //粗暴的下采样
    {
      if (added_pt.x * added_pt.x + added_pt.y * added_pt.y + added_pt.z * added_pt.z > (blind * blind) )   //判断点是否在盲区外，在的话将点push到pl_surf
      {
        pl_surf.points.push_back(added_pt);
      }
    }
